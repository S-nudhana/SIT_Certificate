...
export default async function getEventRegenerateCertificateGenerate(c: Context) {
    let tempDir = ""
    try {
        ...
        const certificateData = await getEventCertificateDownloadModel(result.data.eventID)
        if (!certificateData || certificateData.length === 0) {
            return c.json({
                message: "No certificates found for the event"
            }, 404)
        }
        ...
        if (!deleteStatus) {
            return c.json({
                message: "Failed to delete existing certificates for the event"
            }, 500)
        }
